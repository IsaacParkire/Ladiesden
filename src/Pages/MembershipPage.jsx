...
export default function MembershipPage() {
  ...
  const navigate = useNavigate();  const handleSelectPlan = (planId) => {
    setSelectedPlan(planId);
    
    if (planId === 'basic') {
      // For basic plan, redirect to registration
      navigate('/register');
    } else {
      // For premium and VIP, navigate to payment page
      const selectedPlanData = membershipPlans.find(plan => plan.id === planId);
      
      if (selectedPlanData) {
        // Create a clean version without React elements for navigation
        const cleanPlanData = {
          id: selectedPlanData.id,
          name: selectedPlanData.name,
          subtitle: selectedPlanData.subtitle,
          price: selectedPlanData.price,
          originalPrice: selectedPlanData.originalPrice,
          billing: selectedPlanData.billing,
          color: selectedPlanData.color,
          borderColor: selectedPlanData.borderColor,
          popular: selectedPlanData.popular,
          description: selectedPlanData.description,
          features: selectedPlanData.features,
          restrictions: selectedPlanData.restrictions
        };
  ...
  return (
    <div className="pt-16 sm:pt-20 md:pt-24 bg-black text-white min-h-screen">      {/* Hero Section */}
      <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6">        <div className="max-w-7xl mx-auto text-center">
          <motion.div
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
            {membershipPlans.map((plan, index) => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, delay: index * 0.2 }}                className={`relative bg-gradient-to-b from-zinc-900 to-black rounded-2xl p-8 border-2 ${
                  plan.popular 
              ...
              >
                ...
                {plan.restrictions.length > 0 && (
                  ...
                  className={`w-full py-4 px-6 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${
                    plan.id === 'basic'
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-zinc-900/50 rounded-2xl p-8 backdrop-blur-sm border border-zinc-800">
              ...
              <div className="md:col-span-3 grid grid-cols-3 gap-4">
                {membershipPlans.map((plan) => (
                  <div key={plan.id} className="text-center">
                    <div className={`w-12 h-12 bg-gradient-to-br ${plan.color} rounded-full flex items-center justify-center mx-auto mb-2`}>
                      {plan.icon}
                    </div>
            ...
            <div className="mt-8 space-y-4">
              {[
                { feature: 'Her Night Events', basic: '❌', premium: '✅', vip: '✅ VIP Access' },
                { feature: 'Her Secrets Access', basic: '❌', premium: '✅', vip: '✅ Full Access' },
                { feature: 'Her Touch Services', basic: 'Standard', premium: 'Priority', vip: 'VIP + Custom' },
                { feature: 'Gallery Access', basic: 'Public', premium: 'Premium', vip: 'VIP Exclusive' },
                { feature: 'Booking Priority', basic: 'Standard', premium: 'Priority', vip: 'Guaranteed' },
                { feature: 'Customer Support', basic: 'Standard', premium: 'Premium', vip: '24/7 Concierge' }
              ].map((row, index) => (
                <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 py-4 border-b border-zinc-800 last:border-b-0">
                  ...
                  <div className="text-center text-zinc-400">{row.basic}</div>
                  <div className="text-center text-yellow-400">{row.premium}</div>
                  <div className="text-center text-purple-400">{row.vip}</div>
                </div>
              ))}
            </div>
          </div>
